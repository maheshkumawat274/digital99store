import type { Category, Product } from "../types";


export const CATEGORIES: Category[] = [
  'AI Chatbot', 'AI Prompts', 'Article', 'Business', 'Canva', 
  'ChatGPT', 'CV / Resume', 'e-Book', 'Email', 'Graphics', 
  'HTML', 'Info', 'Quotes Images', 'Short Videos', 'Stock Image', 
  'Tools', 'Training', 'WordPress'
];

export const PRODUCTS: Product[] = [
  {
    id: '1',
    title: 'Modern AI Chatbot Template',
    description: 'A fully responsive AI chatbot UI with dark mode support.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 499,
    category: 'AI Chatbot',
    thumbnail: 'https://picsum.photos/seed/ai1/600/400',
    rating: 4.8,
    sales: 120,
    isBestSeller: true
  },
  {
    id: '2',
    title: 'Ultimate ChatGPT Prompt Pack',
    description: '1000+ expert-crafted prompts for marketing and SEO.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 299,
    category: 'ChatGPT',
    thumbnail: 'https://picsum.photos/seed/chat/600/400',
    rating: 4.9,
    sales: 850,
    isTrending: true,
    isBestSeller: true
  },
  {
    id: '3',
    title: 'Canva Social Media Kit',
    description: '500+ templates for Instagram, LinkedIn, and Twitter.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 199,
    category: 'Canva',
    thumbnail: 'https://picsum.photos/seed/canva/600/400',
    rating: 4.7,
    sales: 320,
    isTrending: true
  },
  {
    id: '4',
    title: 'Professional CV Bundle',
    description: '10 ATS-friendly resume templates for tech professionals.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 199,
    category: 'CV / Resume',
    thumbnail: 'https://picsum.photos/seed/cv/600/400',
    rating: 4.5,
    sales: 210
  },
  {
    id: '5',
    title: 'Digital Marketing Mastery e-Book',
    description: 'The ultimate guide to scaling your online presence.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 349,
    category: 'e-Book',
    thumbnail: 'https://picsum.photos/seed/ebook/600/400',
    rating: 5.0,
    sales: 540,
    isBestSeller: true
  },
  {
    id: '6',
    title: 'WordPress SaaS Landing Page',
    description: 'High-conversion Elementor template for SaaS products.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 599,
    category: 'WordPress',
    thumbnail: 'https://picsum.photos/seed/wp/600/400',
    rating: 4.6,
    sales: 88
  },
  {
    id: '7',
    title: 'Premium Stock Photo Bundle',
    description: '100+ high-res nature and business stock images.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 199,
    category: 'Stock Image',
    thumbnail: 'https://picsum.photos/seed/stock/600/400',
    rating: 4.9,
    sales: 150,
    isTrending: true
  },
  {
    id: '8',
    title: 'Email Marketing Sequences',
    description: 'Proven cold email and newsletter templates.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 449,
    category: 'Email',
    thumbnail: 'https://picsum.photos/seed/email/600/400',
    rating: 4.4,
    sales: 430,
    isTrending: true
  },

  // 🔥 Extra Trending Products
  {
    id: '9',
    title: 'AI Content Writing Toolkit',
    description: 'Ready-to-use AI tools for blogs and ads.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 399,
    category: 'Tools',
    thumbnail: 'https://picsum.photos/seed/ai-tools/600/400',
    rating: 4.8,
    sales: 390,
    isTrending: true
  },
  {
    id: '10',
    title: 'Short Video Reels Pack',
    description: 'Editable short video templates for Instagram Reels.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 349,
    category: 'Short Videos',
    thumbnail: 'https://picsum.photos/seed/reels/600/400',
    rating: 4.6,
    sales: 410,
    isTrending: true
  },

  // ⭐ Extra Best Sellers
  {
    id: '11',
    title: 'Business Proposal Templates',
    description: 'High-converting business proposal designs.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 299,
    category: 'Business',
    thumbnail: 'https://picsum.photos/seed/business/600/400',
    rating: 4.9,
    sales: 620,
    isBestSeller: true
  },
  {
    id: '12',
    title: 'HTML Landing Page Pack',
    description: 'Clean and responsive HTML landing pages.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 299,
    category: 'HTML',
    thumbnail: 'https://picsum.photos/seed/html/600/400',
    rating: 4.7,
    sales: 480,
    isBestSeller: true
  },
  {
    id: '13',
    title: 'Modern AI Chatbot Template',
    description: 'A fully responsive AI chatbot UI with dark mode support.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 499,
    category: 'AI Chatbot',
    thumbnail: 'https://picsum.photos/seed/ai1/600/400',
    rating: 4.8,
    sales: 120,
    isBestSeller: true
  },{
    id: '14',
    title: 'Modern AI Chatbot Template',
    description: 'A fully responsive AI chatbot UI with dark mode support.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 499,
    category: 'AI Chatbot',
    thumbnail: 'https://picsum.photos/seed/ai1/600/400',
    rating: 4.8,
    sales: 120,
    isBestSeller: true
  },{
    id: '15',
    title: 'Modern AI Chatbot Template',
    description: 'A fully responsive AI chatbot UI with dark mode support.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 499,
    category: 'AI Chatbot',
    thumbnail: 'https://picsum.photos/seed/ai1/600/400',
    rating: 4.8,
    sales: 120,
    isBestSeller: true
  },{
    id: '16',
    title: 'Modern AI Chatbot Template',
    description: 'A fully responsive AI chatbot UI with dark mode support.',
     longDescription: "Yahan tum detail me likh sakte ho — features, benefits, compatibility, FAQs, sab kuch.",
    price: 499,
    category: 'AI Chatbot',
    thumbnail: 'https://picsum.photos/seed/ai1/600/400',
    rating: 4.8,
    sales: 120,
    isBestSeller: true
  },
  
];
